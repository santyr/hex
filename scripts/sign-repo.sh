...
cat > "$ARCHON_DIR/manifest.json" << MANIFEST
{
  ...
  "repository": "https://github.com/lightning-goats/hex",
  "files": [
$(find . -type f ! -path "./.git/*" ! -name "manifest*.json" | sort | while read f; do
...
echo "=== Done ==="
echo "Manifest signed. Don't forget to commit and push!"
